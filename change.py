...
if __name__ == "__main__":
    ...
    argparser.add_argument("album", help="String to scan category for.")
        ...
        if album["Title"] == args.album and album["Category"]["id"] == cat_id and album["SubCategory"]["id"] == subcat_id:
            ...
            for photo in photos["Album"]["Images"]:
                ...
                if photo["Date"].find("2011-") >= 0:
                    logging.info("Moving photo %s to Album ID 26312613" % photo["id"])
                    thing = smugmug_request("smugmug.images.changeSettings", {"SessionID": session, "ImageID": photo["id"], "AlbumID": "26312613"})
